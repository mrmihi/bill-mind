    ...
    var body: some View {
        Form {
            Section("Summary") {
                ...
                DetailRow(
                    label: "Due",
                    value: dateFormatter.string(from: bill.date)
                )
                DetailRow(label: "Frequency", value: bill.frequency.rawValue)
                DetailRow(
                    label: "Status",
                    value: bill.isPaid
                        ? "Paid" : bill.isOverdue ? "Overdue" : "Unpaid",
                    valueColor: bill.isPaid
                        ? .green : bill.isOverdue ? .orange : .primary
                )
                DetailRow(label: "Payment", value: bill.paymentMode.rawValue)
            }
            if bill.hasReceipt, let data = bill.receiptData,
