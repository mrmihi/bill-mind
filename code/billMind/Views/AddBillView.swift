...
struct AddBillView: View {
    ...
    @State private var frequency: Bill.Frequency = .none
        ...
        NavigationStack {
            Form {
                ...
                Section("Details") {
                    ...
                    TextField("Amount (LKR)", text: $amountText).keyboardType(
                        .decimalPad
                    )
                    DatePicker(
                        "Due date",
                ...
                Section("Category") {
                    Picker("Category", selection: $category) {
                        ForEach(Bill.Category.allCases) {
                ...
                Section("Payment Mode") {
                    Picker("Payment", selection: $paymentMode) {
                        ForEach(Bill.PaymentMode.allCases) {
                ...
                Section("Repeat") {
                    Picker("Frequency", selection: $frequency) {
                        ForEach(Bill.Frequency.allCases) {
                            Text($0.rawValue).tag($0)
                        }
                    }
                }
                Section("Receipt (optional)") {
                    if let data = receiptData, let img = UIImage(data: data) {
                        Image(uiImage: img).resizable().scaledToFit().frame(
                            maxHeight: 160
                        )
                    }
                    PhotosPicker(selection: $receiptItem, matching: .images) {
                        Label("Select Photo", systemImage: "photo")
                    }.onChange(of: receiptItem) { item in
                        Task {
                            if let d = try? await item?.loadTransferable(
                                type: Data.self
                            ) {
                                receiptData = d
                            }
